...
def load_theme_name() -> str:
    ...
    try:
        ...
        return preset if preset in valid else "gold"
    except BaseException:
        return "gold"
...
# ─────────────────────────────────────────────
# 解約理由（Google Form の選択肢）
# 事前定義された19項目。それ以外は「その他」= 自由記述扱い。
# ─────────────────────────────────────────────
PREDEFINED_CANCEL_REASONS: list[str] = [
    "お試し",
    "高い",
    "余った",
    "実感できない",
...
# 自由記述をキーワードでバケツ分けするルール（上から順に評価、最初にマッチしたものを採用）
FREE_REASON_KEYWORD_RULES: list[tuple[str, list[str]]] = [
    ("認識違い・誤注文", [
        "勘違い", "間違え", "間違って", "思っていた", "思ってた",
        "定期ではな", "通常コースだと", "認識", "誤って",
    ]),
    # hajuCS 特有：肝サプリで「肝臓・肝機能の数値が悪くないから不要」パターン
    ("肝臓数値OK", [
        "肝臓は悪くない", "肝臓が悪くない", "肝臓悪くない",
        "肝臓や",  # 「肝臓や尿酸値の値に効くのをみて不要」
        "肝機能",  # 「肝機能などは悪くない」「肝機能の数値に問題がなかった」
        "健常値", "健康診断", "数値に問題がな",
    ]),
    # hajuCS 特有：ダイエット目的で購入した（＝肝ケア目的ではなかった）パターン
    ("ダイエット目的", [
        "ダイエット", "痩せたい", "減量",
    ]),
    # hajuCS 特有：おなら/便通改善を期待していたパターン
    ("おなら・便通", [
        "おなら", "ガス", "便秘", "下痢", "便通", "排便",
    ]),
    ("家族関係", [
    ...
    ("飲用習慣", [
        "飲まなく", "飲めない", "飲むの忘れ", "飲み忘れ", "飲まない",
        "お酒", "飲酒", "禁酒", "断酒",
        "続かない", "続けられ", "定期的に飲",
    ]),
    ("応対不満", [
        "対応が不満", "応対不満", "接客", "対応不満", "対応が悪",
    ]),
    ("必要性なし", [
        "必要ない", "必要なくな", "不要", "やめたい", "やっぱりやめ",
    ]),
    ("電話対応困難", [
        "代理", "伺えず", "電波", "切電", "不通", "聞き取り前",
    ]),
    ("諸事情", [
        "諸事情", "事情", "とりあえず", "いったん", "一旦", "訳あって",
    ]),
    ("施設・入院", [
        "施設", "入院", "介護", "老人ホーム", "ホーム入",
    ]),
    ("様子見・迷い", [
        "様子見", "様子を見", "迷って", "一度止め", "一度やめ", "保留", "考え直",
    ]),
    ("支払い関連", [
        "支払", "未払", "弁護士", "督促", "請求",
    ]),
...
def classify_free_cancel_reason(text: str) -> str:
    """解約理由の自由記述を分類する。

    - 事前定義カテゴリと一致する場合は空文字を返す（＝別集計へ）
    - キーワードルールに一致すれば、そのカテゴリ名
    - どのルールにも一致しなければ「その他(分類不能)」
    """
    if not isinstance(text, str) or not text.strip():
        return ""
    t = text.strip()
    if t in PREDEFINED_CANCEL_REASONS:
        return ""
    for category, keywords in FREE_REASON_KEYWORD_RULES:
        for kw in keywords:
            if kw in t:
                return category
    return "その他(分類不能)"
